...
public class VerifyPhoneNumber implements RequiredActionProvider, RequiredActionFactory {
    ...
    public void requiredActionChallenge(RequiredActionContext context) {
        ...
        if (! Objects.equals(authSession.getAuthNote(Constants.VERIFY_PHONE_NUMBER_KEY), phoneNumber)) {
            ...
            challenge = sendVerifyPhoneNumber(context.getSession(), loginFormsProvider, context.getUser(), context.getAuthenticationSession(), event);
        } else {
            challenge = loginFormsProvider.createResponse(UserModel.RequiredAction.VERIFY_PHONE_NUMBER);
        }
    ...
    public void processAction(RequiredActionContext context) {
        ...
        context.getAuthenticationSession().removeAuthNote(Constants.VERIFY_PHONE_NUMBER_KEY);
    ...
    private Response sendVerifyPhoneNumber(KeycloakSession session, LoginFormsProvider forms, UserModel user, AuthenticationSessionModel authSession, EventBuilder event) throws UriBuilderException, IllegalArgumentException {
        RealmModel realm = session.getContext().getRealm();
        UriInfo uriInfo = session.getContext().getUri();

        int validityInSecs = realm.getActionTokenGeneratedByUserLifespan(VerifyPhoneNumberActionToken.TOKEN_TYPE);
        ...
        try {
            session
              ...
              .setRealm(realm)
              .setUser(user)
              .sendVerifyPhoneNumber(link, expirationInMinutes);
            event.success();
        ...
        return forms.createResponse(UserModel.RequiredAction.VERIFY_PHONE_NUMBER);
    }
}
