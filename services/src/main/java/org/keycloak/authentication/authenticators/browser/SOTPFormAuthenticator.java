...
import org.keycloak.credential.SOTPCredentialProvider;
import org.keycloak.credential.ApplessOTPCredentialProviderFactory;
 ...
 * @version $Revision: 1 $
 */
public class SOTPFormAuthenticator extends AbstractUsernameFormAuthenticator implements Authenticator, CredentialValidator<SOTPCredentialProvider> {
    ...
    protected Response createLoginForm(LoginFormsProvider form) {
        return form.createLoginSotp();
    }

    @Override
    public boolean configuredFor(KeycloakSession session, RealmModel realm, UserModel user) {
        System.out.println("!!!!!!!!!!!IS CONFIGURED FOR? : at SOTP " + getCredentialProvider(session).getType());
        System.out.println("!!!!!!!!!!!IS it configured? at SOTP " + user.credentialManager().isConfiguredFor(getCredentialProvider(session).getType()));
    ...
    public void setRequiredActions(KeycloakSession session, RealmModel realm, UserModel user) {
        ...
        authenticationSession.setAuthNote(Details.OVERRIDE_VERIFICATION, "true");
        if (!authenticationSession.getRequiredActions().contains(UserModel.RequiredAction.UPDATE_PHONE_NUMBER.name())) {
            authenticationSession.addRequiredAction(UserModel.RequiredAction.UPDATE_PHONE_NUMBER);
        }
    }

    public List<RequiredActionFactory> getRequiredActions(KeycloakSession session) {
        return Collections.singletonList((UpdatePhoneNumber)session.getKeycloakSessionFactory().getProviderFactory(RequiredActionProvider.class, UserModel.RequiredAction.UPDATE_PHONE_NUMBER.name()));
    }
    ...
    @Override
    public SOTPCredentialProvider getCredentialProvider(KeycloakSession session) {
        return (SOTPCredentialProvider)session.getProvider(CredentialProvider.class, ApplessOTPCredentialProviderFactory.PROVIDER_ID);
    }
